...
import com.yovvis.easyopencommon.common.*;
...
import com.yovvis.easyopenmodel.dto.app.AppAddRequest;
import com.yovvis.easyopenmodel.dto.app.AppEditRequest;
import com.yovvis.easyopenmodel.dto.app.AppQueryRequest;
import com.yovvis.easyopenmodel.dto.app.AppUpdateRequest;
...
public class AppController {
    ...
    public BaseResponse<Boolean> deleteApp(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        // 仅本人或管理员可删除
        if (!oldApp.getUserId().equals(user.getId()) && !userFeignClient.isAdmin(user)) {
    ...
    public BaseResponse<Boolean> editApp(@RequestBody AppEditRequest appEditRequest, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(oldApp == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldApp.getUserId().equals(loginUser.getId()) && !userFeignClient.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        // 重置审核状态
        app.setReviewStatus(ReviewStatusEnum.REVIEWING.getValue());
