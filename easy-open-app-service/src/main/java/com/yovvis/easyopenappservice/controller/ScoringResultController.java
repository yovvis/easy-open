...
import com.yovvis.easyopenmodel.dto.scoringresult.ScoringResultAddRequest;
import com.yovvis.easyopenmodel.dto.scoringresult.ScoringResultEditRequest;
import com.yovvis.easyopenmodel.dto.scoringresult.ScoringResultQueryRequest;
import com.yovvis.easyopenmodel.dto.scoringresult.ScoringResultUpdateRequest;
...
public class ScoringResultController {
    ...
    public BaseResponse<Boolean> deleteScoringResult(@RequestBody DeleteRequest deleteRequest,
        ...
        // 仅本人或管理员可删除
        if (!oldScoringResult.getUserId().equals(user.getId()) && !userFeignClient.isAdmin(user)) {
    ...
    public BaseResponse<Boolean>
        updateScoringResult(@RequestBody ScoringResultUpdateRequest scoringResultUpdateRequest) {
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Page<ScoringResult>>
        listScoringResultByPage(@RequestBody ScoringResultQueryRequest scoringResultQueryRequest) {
    ...
    public BaseResponse<Boolean> editScoringResult(@RequestBody ScoringResultEditRequest scoringResultEditRequest,
        ...
        ThrowUtils.throwIf(oldScoringResult == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldScoringResult.getUserId().equals(loginUser.getId()) && !userFeignClient.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        // 操作数据库
        boolean result = scoringResultService.updateById(scoringResult);
